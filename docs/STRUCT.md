...
- `src/` - Source code
    - `agent/`
        ...
        - `prompts.py` - Manages system and user prompts (legacy)
        - `form_processor.py` - Orchestrates form filling and multi-page flows (legacy)
        - `zero_actions_handler.py` - Handles edge cases using DOM analysis and vision fallback (legacy)
        - `vision_fallback.py` - Uses Claude vision to find elements when DOM detection fails (legacy)
    ...
    - `ats/` (Legacy - No longer used in main flows)
